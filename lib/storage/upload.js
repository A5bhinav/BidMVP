 ...
 * Can be used during event creation (with userId) or after (with eventId)
 ...
 */
export async function uploadEventImage(eventId, file, userId = null) {
  try {
    const supabase = await createClient()
    ...
    // Validate userId is provided when eventId is not (for creation)
    if (!eventId && !userId) {
      return {
        data: null,
        error: { message: 'User ID is required for event image upload during creation' }
      }
    }

    // For event creation, use userId in temp path
    // For updates, use eventId
    const timestamp = Date.now()
    const fileName = eventId
      ? `${eventId}/${timestamp}-${file.name}`
      : `temp/${userId}/${timestamp}-${file.name}`
    ...
    return { data: null, error: { message: error.message || 'Failed to upload event image' } }
  }
}
