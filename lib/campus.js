...
export async function autoLinkUser(userId, email) {
  try {
    ...
    if (existingUser?.school_id) {
      ...
      // Only select columns that exist to avoid schema cache errors
      ...
      if (school) {
        // Add null values for optional columns
    ...
    if (!school) {
      ...
      const { data: newSchool, error: createError } = await createSchool({
        name: schoolName,
        domain: domain
        // Note: abbreviation not included as column may not exist in database
      })
