...
export async function getEventsPendingCounts(eventIds, adminUserId) {
  try {
    ...
    // Verify admin has access to all events' fraternities
    const fratIds = [...new Set(events.map(e => e.frat_id))]
    for (const fratId of fratIds) {
      const { data: adminCheck, error: adminError } = await checkIsAdmin(adminUserId, fratId)
      if (adminError || !adminCheck?.isAdmin) {
