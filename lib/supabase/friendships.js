...
export async function sendFriendRequest(userId, friendId) {
  try {
    ...
    // Validate that both users exist in the User table
    ...
    if (friendError || !friend) {
      return { data: null, error: { message: 'Friend user not found' } }
    }
...
export async function blockUser(userId, blockedUserId) {
  try {
    ...
    if (blockedUserError || !blockedUser) {
      return { data: null, error: { message: 'User to block not found' } }
    }
    ...
    if (existingFriendship) {
      ...
      // IMPORTANT: Do NOT swap user_id/friend_id — this would break RLS policies
      // that check auth.uid() = user_id for row ownership.
      // Instead, only update status and track who initiated the block via the status itself.
