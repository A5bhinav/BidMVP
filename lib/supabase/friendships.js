...
export async function getPeopleYouMet(userId, limit = 20, includeCrossSchool = true) {
  try {
    ...
    // This is more complex but works without the database function
    ...
    if (checkinError) {
      return { data: null, error: serializeError(checkinError, 'Failed to get user check-ins') }
    }
    ...
    if (otherError) {
      return { data: null, error: serializeError(otherError, 'Failed to get other users check-ins') }
    }
    ...
    // Get user profiles for suggested users (include school_id for filtering)
    const suggestedUserIds = Array.from(userEventMap.keys())
    ...
    // Filter by school if needed and prioritize same-school users
    let filteredUsers = users
    if (!includeCrossSchool && userSchoolId) {
      // Only include users from same school
      filteredUsers = users.filter(user => user.school_id === userSchoolId)
    }

    // Combine user data with event counts and add school match flag
    const suggestions = filteredUsers
      .map(user => {
        ...
        return {
          ...
          lastEventDate: null, // Timestamp not available - sorting by event count only
