...
export async function createFraternity(fraternityData, creatorUserId) {
  try {
    ...
    if (!fraternityData.type || !['fraternity', 'sorority', 'other'].includes(fraternityData.type)) {
    ...
    // Get creator's school_id for duplicate check
    ...
    // Check for duplicate name at same school
    let duplicateWarning = false
    if (creatorProfile?.school_id) {
      // Get all fraternities at this school
      const { data: existingFraternities } = await supabase
        .from('fraternity')
        .select('id, name')
        .ilike('name', fraternityData.name.trim())

      // Check if any existing fraternity has members from the same school
      if (existingFraternities && existingFraternities.length > 0) {
        for (const existing of existingFraternities) {
          ...
          if (members && members.length > 0) {
            ...
            if (memberUser?.school_id === creatorProfile.school_id) {
    ...
    const { data: fraternity, error: fraternityError } = await supabase
      ...
      .insert({
        ...
        flagged_for_review: duplicateWarning
      })
      .select()
      .single()
    ...
    if (memberError) {
      ...
      return { data: null, error: { message: 'Failed to add creator as admin member' } }
    }

    // Return with duplicate warning if applicable
    if (duplicateWarning) {
      return {
        data: fraternity,
        error: {
          message: 'Duplicate name warning',
          code: 'DUPLICATE_NAME',
          allowCreation: true
        }
      }
...
export async function searchFraternities(query, schoolId, limit = 20) {
  try {
    ...
    // Only return verified fraternities OR fraternities with 7+ quality members
    queryBuilder = queryBuilder.or('verified.eq.true,member_verified.eq.true')
    ...
    // Filter by school if schoolId provided
    if (schoolId && fraternities) {
      const filtered = []
      for (const fraternity of fraternities) {
        // Check if any member of this fraternity is from the specified school
        const { data: members } = await supabase
          .from('group_members')
          .select('user_id')
          .eq('group_id', fraternity.id)
          .limit(1)
        ...
        if (members && members.length > 0) {
          ...
          if (memberUser?.school_id === schoolId) {
            filtered.push(fraternity)
          }
        }
      }
