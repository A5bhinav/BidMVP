...
export async function createFraternity(fraternityData, creatorUserId) {
  try {
    ...
    if (memberError) {
      // Log detailed error for debugging - use JSON.stringify to see full error structure
      try {
        console.error('Failed to add creator as admin member:', {
          errorType: typeof memberError,
          errorConstructor: memberError?.constructor?.name,
          errorString: String(memberError),
          errorJSON: JSON.stringify(memberError, Object.getOwnPropertyNames(memberError)),
      ...
      const serializedError = serializeError(memberError, 'Failed to add creator as admin member')
      return { 
        data: null, 
        error: {
          message: serializedError.message,
          details: serializedError.details || serializedError.hint || null,
 ...
 * Includes fraternities from group_members AND fraternities where user is creator
 * @param {string} userId - User ID
...
export async function getUserFraternities(userId) {
  try {
    ...
    // Get fraternities from group_members table (standard membership)
    ...
    // Get fraternities where user is creator (fallback for cases where RPC failed)
    const { data: createdFraternities, error: creatorError } = await supabase
      .from('fraternity')
      .select(`
        id,
        name,
    ...
    if (creatorError) {
      // Log but don't fail - creator check is a fallback
      console.error('Error fetching creator fraternities:', creatorError)
    }

    // Create a map of fraternity IDs from memberships to avoid duplicates
    const membershipFraternityIds = new Set((memberships || []).map(m => m.fraternity?.id).filter(Boolean))

    // Transform memberships to expected format
    const fraternitiesFromMemberships = (memberships || []).map(m => ({
      fraternity: m.fraternity,
      role: m.role
    }))

    // Add creator fraternities that aren't already in memberships (with admin role)
    const creatorFraternities = (createdFraternities || [])
      .filter(f => !membershipFraternityIds.has(f.id))
      .map(f => ({
        fraternity: f,
        role: 'admin' // Creator should have admin role
      }))

    // Combine both lists
    const allFraternities = [...fraternitiesFromMemberships, ...creatorFraternities]

    return { data: allFraternities, error: null }
  } catch (error) {
