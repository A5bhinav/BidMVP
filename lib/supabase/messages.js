...
export async function getConversations(userId) {
  try {
    ...
    if (conversationIds.length > 0) {
      // Fetch recent messages — limit is bounded to avoid excessive data transfer
      // We only need the latest message per conversation, but Supabase doesn't support
      // DISTINCT ON via the client API, so we fetch a reasonable fixed amount and deduplicate
      const messageLimit = Math.min(conversationIds.length * 2, 200)
...
export async function getUnreadCount(userId) {
  try {
    ...
    // Only query if we have conversations
    if (conversationIds.length === 0) {
      return { data: { total_unread: 0 }, error: null }
    }

    // Use count-only query instead of fetching all unread message rows
    // This is dramatically more efficient - transfers only the count, not all rows
