...
export async function createEvent(eventData, creatorUserId) {
  try {
    ...
    */
    ...
    // Optional: Validate lat/lng format if user provides coordinates directly
    const latLngPattern = /^-?\d+\.?\d*,-?\d+\.?\d*$/
    if (latLngPattern.test(locationValue)) {
      // User provided coordinates directly (alternative method)
      ...
      // User provided address (preferred method)
      // Address will be geocoded when needed for radius checking
      // Store address as-is in database
    }
