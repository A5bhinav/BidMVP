...
export async function createEvent(eventData, creatorUserId) {
  try {
    ...
    // Validate fraternity is verified before allowing event creation
...
export async function getEvent(eventId) {
  try {
    ...
    if (event) {
      ...
      if (event.bid_price !== null && event.bid_price !== undefined) {
        event.bid_price = parseFloat(event.bid_price)
      }
      if (event.line_skip_price !== null && event.line_skip_price !== undefined) {
        event.line_skip_price = parseFloat(event.line_skip_price)
      }
      // max_bids may not exist if migration hasn't been run - default to null (unlimited)
      if (event.max_bids !== null && event.max_bids !== undefined) {
        event.max_bids = parseInt(event.max_bids, 10)
      } else {
        event.max_bids = null
      }
      if (event.location_lat !== null && event.location_lat !== undefined) {
        event.location_lat = parseFloat(event.location_lat)
      }
      if (event.location_lng !== null && event.location_lng !== undefined) {
        event.location_lng = parseFloat(event.location_lng)
      }
    }
...
export async function getCampusEvents(userId, filters = {}) {
  try {
    ...
    // Build query
    // Note: max_bids column may not exist if migration 001_schema_extensions.sql hasn't been run
    // If column doesn't exist, we'll handle it in the formatting step
    let query = supabase
      .from('event')
      .select(`
        id,
    ...
    // Apply visibility rules at the database level (not client-side)
    // This ensures correct pagination and reduces data transfer

    // If user is NOT rushing, exclude rush-only events from results
    if (!user.rushing) {
      query = query.neq('visibility', 'rush-only')
    }

    // Apply rush_only filter: if rush_only=true, only show rush-only events
    if (filters.rush_only === true) {
      query = query.eq('visibility', 'rush-only')
    }
    ...
    // No client-side filtering needed — all visibility rules applied at DB level
