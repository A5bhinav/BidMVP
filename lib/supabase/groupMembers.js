...
export async function addMember(groupId, userId, role = 'member', addedByUserId) {
  try {
    ...
    // Permission check: Only admins can add members
    ...
    // Get updated quality member count (trigger should have updated it)
    const { data: fraternity } = await supabase
      .from('fraternity')
      .select('quality_member_count, member_verified, verified')
      .eq('id', groupId)
      .single()
