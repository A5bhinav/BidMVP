...
export async function checkIsAdmin(userId, groupId) {
  try {
    ...
    // First, check if user is the creator of the fraternity
    // This is a direct check that works regardless of migration status
    ...
    // If user is the creator, they're definitely an admin
    if (!fraternityError && fraternity?.creator_id === userId) {
      return { data: { isAdmin: true }, error: null }
    }

    // Also check via RPC function (checks group_members table and creator_id if migration applied)
    ...
    if (error) {
      // If RPC fails, we already checked creator_id above, so return false
